...
def test_toml_config_loaded(tmp_path, monkeypatch):
    ...
    config_dir = tmp_path / ".config" / "reprompt"
    ...
    monkeypatch.delenv("REPROMPT_EMBEDDING_BACKEND", raising=False)
    monkeypatch.delenv("REPROMPT_DEDUP_THRESHOLD", raising=False)
    monkeypatch.delenv("REPROMPT_OLLAMA_URL", raising=False)
...
def test_env_overrides_toml(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("REPROMPT_EMBEDDING_BACKEND", "tfidf")
    monkeypatch.delenv("REPROMPT_DEDUP_THRESHOLD", raising=False)
...
def test_toml_missing_file_uses_defaults(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("REPROMPT_CONFIG_PATH", str(tmp_path / "nonexistent.toml"))
    monkeypatch.delenv("REPROMPT_EMBEDDING_BACKEND", raising=False)
    monkeypatch.delenv("REPROMPT_DEDUP_THRESHOLD", raising=False)
...
def test_toml_empty_section_uses_defaults(tmp_path, monkeypatch):
    """TOML file exists but has no [reprompt] section."""
    config_dir = tmp_path / ".config" / "reprompt"
    config_dir.mkdir(parents=True)
    config_file = config_dir / "config.toml"
