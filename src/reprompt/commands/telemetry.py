...
def _telemetry_config_path() -> Path:
    ...
    env_path = os.environ.get("REPROMPT_CONFIG_PATH")
    if env_path:
        return Path(env_path)

    from reprompt.config import _default_config_path
...
def telemetry_status() -> None:
    ...
    console.print()
    console.print(
        "Manage: [bold]reprompt telemetry on[/bold] / [bold]reprompt telemetry off[/bold]"
    )
