    ...
    if config_path.is_file():
        try:
            with open(config_path, "rb") as f:
                data = tomllib.load(f)
            existing = data.get("reprompt", {}).get("telemetry_salt")
            if existing:
                return str(existing)
        except Exception:
            pass
...
def read_consent(config_path: Path) -> TelemetryConsent:
    ...
    try:
        with open(config_path, "rb") as f:
            data = tomllib.load(f)
        raw = data.get("reprompt", {}).get("telemetry_consent", "not_asked")
...
def _upsert_toml_key(config_path: Path, key: str, value: str) -> None:
    """Insert or update a key under [reprompt] in the TOML config file.
    ...
    # Ensure [reprompt] section exists
    if "[reprompt]" not in content:
        if content and not content.endswith("\n"):
            content += "\n"
        content += "[reprompt]\n"

    # Check if key already exists under [reprompt]
    lines = content.split("\n")
    found = False
    ...
    if not found:
        # Insert after [reprompt] line
        for i, line in enumerate(lines):
            if line.strip() == "[reprompt]":
