...
2. .ctxray.toml in CWD or parents
3. [tool.ctxray.lint] in pyproject.toml
4. Built-in defaults
"""
...
@dataclass
class LintConfig:
    """Configuration for lint rules. Loaded from .ctxray.toml or pyproject.toml."""
...
def load_lint_config(start_dir: Path | None = None) -> LintConfig:
    """Load lint config from .ctxray.toml or pyproject.toml, walking up from start_dir.
    ...
    for _ in range(20):  # safety limit
        # Check .ctxray.toml first (project-specific)
        ctxray_toml = current / ".ctxray.toml"
        if ctxray_toml.is_file():
            return _parse_ctxray_toml(ctxray_toml)
...
def _parse_ctxray_toml(path: Path) -> LintConfig:
    """Parse a .ctxray.toml file."""
    try:
...
def _parse_pyproject_toml(path: Path) -> LintConfig | None:
    """Parse [tool.ctxray.lint] from pyproject.toml. Returns None if section missing."""
    try:
        with open(path, "rb") as f:
            data = tomllib.load(f)
        lint_section = data.get("tool", {}).get("ctxray", {}).get("lint")
...
def lint_prompt(text: str, config: LintConfig | None = None) -> list[LintViolation]:
    ...
    if config.max_tokens > 0:
        from ctxray.core.cost import estimate_tokens
            ...
            violations.append(
                LintViolation(
                    ...
                    message=(
                        ...
                        "consider compressing with `ctxray compress`"
                    ),
                    prompt_text=text,
                )
            )
