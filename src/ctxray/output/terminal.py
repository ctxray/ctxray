...
def render_report(data: dict[str, Any]) -> str:
    ...
    # Title
    console.print("\n[bold]ctxray — AI Session Analytics[/bold]")
    console.print("=" * 40)
    ...
    if data.get("privacy") and data["privacy"]["total_prompts"] > 0:
        ...
        console.print("  Run `ctxray privacy` for full breakdown")

    console.print("\nRun `ctxray template list` to see your reusable prompt collection")
    console.print("Run `ctxray digest --trends` to see your prompt evolution over time")
...
def render_trends(data: dict[str, Any]) -> str:
    ...
    console.print("\n[bold]ctxray trends — Prompt Evolution[/bold]")
    console.print("=" * 40)
...
def render_recommendations(data: dict[str, Any]) -> str:
    ...
    console.print("\n[bold]ctxray recommend — Prompt Improvement Tips[/bold]")
    console.print("=" * 40)
...
def render_templates(templates: list[dict[str, Any]], category_filter: str | None = None) -> str:
    ...
    if not templates:
        ...
        console.print('Run [bold]ctxray template save "your prompt"[/bold] to save one.')
        return buf.getvalue()
...
def render_merge_view(data: dict[str, Any]) -> str:
    ...
    console.print("\n[bold]ctxray merge-view — Similar Prompt Clusters[/bold]")
    console.print("=" * 40)
    ...
    if not clusters:
        ...
        console.print("Run [bold]ctxray scan[/bold] to index more sessions.")
        return buf.getvalue()
    ...
    if count > 0:
        console.print(
            "Run [bold]ctxray template save[/bold] to save ★ prompts as reusable templates."
        )
...
def render_insights(data: dict[str, Any]) -> str:
    ...
    if count == 0:
        console.print(
            "\n[dim]No prompt data yet. Run 'ctxray scan' first, then 'ctxray insights'.[/dim]"
        )
        return buf.getvalue()
...
def render_digest(data: dict[str, Any]) -> str:
    ...
    console.print(f"\n[bold]ctxray digest — Weekly Summary ({period})[/bold]")
    console.print("=" * 40)
...
def render_digest_history(rows: list[dict[str, Any]], period: str) -> str:
    ...
    console.print(f"\n[bold]ctxray digest history ({period})[/bold]")
    console.print("=" * 40)

    if not rows:
        console.print("  No digest history found. Run `ctxray digest` to generate one.")
        return buf.getvalue()
...
def render_style(data: dict[str, Any]) -> str:
    ...
    if data["prompt_count"] == 0:
        console.print("\n[dim]No prompts yet. Run 'ctxray scan' or 'ctxray import' first.[/dim]")
        return buf.getvalue()
...
def render_privacy(data: dict[str, Any]) -> str:
    ...
    if total == 0:
        console.print("\n[dim]No prompt data yet. Run 'ctxray scan' first.[/dim]")
        return buf.getvalue()
...
def render_privacy_deep(scan_result: Any) -> str:
    ...
    console.print("\n[dim]Run `ctxray privacy --deep --json` for full details.[/dim]")
